  ...
  for (i in 1:ND) {
    ...
    if (i > 1) {
      if (phold.orig=="custom1") {
        ...
        threshold <- 100
        val <- 0.99    # previously 0.995 for constant; 0.99 for adjusting based
        # on the 0.995 upper value in this example customization, where we
        # start ramping down phold once the number of follicles that are
        # growth-activated falls below the threshold.
        num_growth_activated_now <- sum(z[,i-1]>0 & probs[,1]==0)
        phold.new <- val + (0.995 - val) *
          min(threshold, num_growth_activated_now) / threshold
      }
